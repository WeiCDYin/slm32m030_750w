...
HAL_StatusTypeDef HAL_LVD_Init(LVD_HandleTypeDef *hlvd)
{
    ...
    assert_param(IS_LVD_TRIGGERSEL(hlvd->Init->TriggerSel));
    assert_param(IS_LVD_FILTER_CLK(hlvd->Init->FltClkSel));
    assert_param(IS_LVD_ACTION(hlvd->Init->Action));
    // LVD->CR = 0;
...
static void LVD_SetConfig(LVD_HandleTypeDef *hlvd)
{
    /* Reset lvd configuration */
    uint32_t regValue;
    CLEAR_BIT(hlvd->Instance->SR, LVD_SR_INTF);
    if (hlvd->Init.FltClkSel != LVD_FLTCLK_DISABLE)
    {
        regValue = hlvd->Init.FltClkSel | (hlvd->Init.FltNum << LVD_CR_FLT_NUM_Pos);
        WRITE_REG(hlvd->Instance->CR, hlvd->Init.VoltageLevel | hlvd->Init.Action | hlvd->Init.TriggerSel | regValue);
        __HAL_LVD_ENABLE_DIGFLT(hlvd);
    }
    else
    {
        WRITE_REG(hlvd->Instance->CR, hlvd->Init.VoltageLevel | hlvd->Init.Action | hlvd->Init.TriggerSel);
    }
}
