...
HAL_StatusTypeDef HAL_BEEP_Init(BEEP_HandleTypeDef *hbeep)
{
    ...
    assert_param(IS_BEEP_INIT_CLKSEL(hbeep->Init.ClkSel));
    assert_param(IS_BEEP_INIT_OUTFREQSEL(hbeep->Init.OutFreqSel));
    assert_param(IS_BEEP_INIT_PREFREQDIV(hbeep->Init.PreFreqDiv));
...
static void BEEP_SetConfig(BEEP_HandleTypeDef *hbeep)
{
    ...
    hbeep->Instance->CSR = hbeep->Init.ClkSel | hbeep->Init.PreFreqDiv | hbeep->Init.OutFreqSel;
}
