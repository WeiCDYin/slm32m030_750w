/**
  ...
  *         
  ...
  [..] This driver provides functions to configure and program the FLASH memory 
       of all slm32x030 devices. It includes
 ...
 * @{
 */
#define FLASH_POSITION_WDGSW_BIT        FLASH_OBR_WDG_SW_Pos
...
HAL_StatusTypeDef HAL_FLASHEx_OBErase(void)
{
    ...
    if(status == HAL_OK)
    {
        ...
        SET_BIT(FLASH->OP_MODE, FLASH_OP_MODE_ERASE_MODE);
...
#elif defined(OB_WRP_PAGES0TO79MASK)
    WRP0_Data = (uint16_t)(WriteProtectPage & OB_WRP_PAGES0TO79MASK);
#endif /* OB_WRP_PAGES0TO79MASK */
...
#elif defined(OB_WRP_PAGES96TO1031MASK)
    WRP3_Data = (uint16_t)((WriteProtectPage & OB_WRP_PAGES96TO1031MASK) >> 24U); 
...
/**
  * @brief  Program the FLASH app Option Byte.    
  * @note   Programming of the OB should be performed only after an erase (otherwise PGERR occurs)
  * @param  UserConfig The FLASH app Option Bytes values FLASH_OBR_WDG_SW(Bit2), 
...
/**
  ...
  *         This parameter can be one of the following values:
  *            @arg @ref OB_RDP_LEVEL_0 No protection
  *            @arg @ref OB_RDP_LEVEL_1 =0xFF（一级保护）
  *            @arg @ref OB_RDP_LEVEL_2 =0xCC（二级保护）
  */
static uint32_t FLASH_OB_GetRDP(void)
{
    ...
    /* Read RDP level bits */
    tmp_reg = READ_BIT(FLASH->OBR, FLASH_OBR_RDPRT) >> FLASH_OBR_RDPRT_Pos;
...
/**
  * @brief  Return the FLASH app Option Byte value.
  * @retval The FLASH app Option Bytes values: FLASH_OBR_WDG_SW(Bit2), 
  *         FLASH_OBR_NRST_STOP(Bit3),FLASH_OBR_NRST_STDBY(Bit4).
  *    
  */
static uint8_t FLASH_OB_GetUser(void)
{
    /* Return the app Option Byte */
    return (uint8_t)((READ_REG(FLASH->OBR) & FLASH_OBR_USER) >> FLASH_POSITION_WDGSW_BIT);
}
