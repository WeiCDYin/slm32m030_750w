/**
  ...
  *           + Peripheral State functions
  ...
  [..]
    ...
    (+) WWDGRST flag in RCMU_CSR register can be used to inform when a WWDG
        reset occurs.
  ...
  ==============================================================================
  [..]
    (+) Enable WWDG APB1 clock using __HAL_RCMU_WWDG_CLK_ENABLE().
    ...
    (+) if Early Wakeup Interrupt mode is enable an interrupt is generated when
        the counter reaches 0x40. App can add his own code in weak function
...
HAL_StatusTypeDef HAL_WWDG_Init(WWDG_HandleTypeDef *hwwdg)
{
    ...
    if(hwwdg->Status == HAL_WWDG_STATE_RESET)
    {
    ...
    hwwdg->Status = HAL_WWDG_STATE_BUSY;

    /* Set WWDG Counter */
    tmpRegVal = hwwdg->Instance->CR;
    tmpRegVal &=((~(WWDG_CR_WDGA | WWDG_CR_T)));
    tmpRegVal |=(WWDG_CR_WDGA | hwwdg->Init.Counter);
    hwwdg->Instance->CR = tmpRegVal;

    /* Set WWDG Prescaler and Window */
    tmpRegVal = hwwdg->Instance->CFR;
    tmpRegVal &=(~(WWDG_CFR_W | WWDG_CFR_WDGTB | WWDG_CFR_EWI));
    tmpRegVal |=(hwwdg->Init.EWIMode | hwwdg->Init.Prescaler | hwwdg->Init.Window);
    hwwdg->Instance->CFR = tmpRegVal;

    hwwdg->Status = HAL_WWDG_STATE_READY;
