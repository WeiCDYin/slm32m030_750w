...
typedef struct {
    ...
    mod_gain_t k_fwd;    /* the modulator's forward gain, taken at the SLOW rate (mod.h) */
    udc_pu_t udc_meas;   /* measured DC link voltage (pu), THIS tick -- deliberately a different
                          * vintage from k_fwd: mod_realized_ref must report what the bridge actually
...
void     cc_tune(cc_t *cc, const motor_cfg_t *m, const base_t *b, const cc_cfg_t *c);
/* reset PI controller */
 ...
 *
 * `dabc_out` and `uab_ref_lim` -- the two results used to arrive inside one returned struct. A struct
 ...
 *
 * `dabc_out` and `uab_ref_lim` must be non-NULL and are NOT checked -- they are the caller's own
...
void cc_step(cc_t *cc, const cc_in_t *in, duties_t *dabc_out, ab_pu_t *uab_ref_lim);
